...
(function () {
  "use strict";
  var el = {
    input: document.getElementById("rot-input"),
    output: document.getElementById("rot-output"),
    status: document.getElementById("rot-status"),
    live: document.getElementById("rot-live"),
    copy: document.getElementById("rot-copy"),
    clear: document.getElementById("rot-clear"),
    b13: document.getElementById("rot-rot13"),
    b5: document.getElementById("rot-rot5"),
    b18: document.getElementById("rot-rot18"),
    resizer: document.getElementById("rot-resizer"),
    left: document.getElementById("rot-left"),
    right: document.getElementById("rot-right"),
    panels: document.getElementById("rot-panels"),
    layout: document.getElementById("rot-layout"),
    layoutStack: document.getElementById("rot-layout-stack"),
    layoutCol: document.getElementById("rot-layout-col"),
    tool: document.getElementById("rot-tool"),
    fullscreen: document.getElementById("rot-fullscreen"),
    fsEnter: document.getElementById("rot-fs-enter"),
    fsExit: document.getElementById("rot-fs-exit")
  };
  var mode = "rot13";

  function rotChar(c, n, base, size) {
    var code = c.charCodeAt(0);
    var rel = code - base;
    return String.fromCharCode(base + ((rel + n) % size + size) % size);
  }
  function transform(text, mode) {
    var out = "", letters = 0, digits = 0;
    for (var i = 0; i < text.length; i++) {
      var c = text[i];
      if (mode !== "rot5" && /[A-Z]/.test(c)) { out += rotChar(c, 13, 65, 26); letters++; }
      else if (mode !== "rot5" && /[a-z]/.test(c)) { out += rotChar(c, 13, 97, 26); letters++; }
      else if (mode !== "rot13" && /[0-9]/.test(c)) { out += rotChar(c, 5, 48, 10); digits++; }
      else out += c;
    }
    return { out: out, letters: letters, digits: digits };
  }

  function update() {
    var raw = el.input.value;
    if (!raw) { el.output.value = ""; setStatus(""); return; }
    var r = transform(raw, mode);
    el.output.value = r.out;
    setStatus('<span class="text-slate-400 dark:text-zinc-500">' + L.letters + ": " + r.letters + " · " + L.digits + ": " + r.digits + "</span>");
  }

  function setMode(m) {
    mode = m;
    el.b13.classList.toggle("active", m === "rot13");
    el.b5.classList.toggle("active", m === "rot5");
    el.b18.classList.toggle("active", m === "rot18");
  ...
  function applyLayout() {
    el.tool.classList.toggle("rot-stacked", stacked);
  ...
  el.b13.addEventListener("click", function () { setMode("rot13"); });
  el.b5.addEventListener("click", function () { setMode("rot5"); });
  el.b18.addEventListener("click", function () { setMode("rot18"); });
  ...
  var pasteBtn = document.getElementById("rot-paste");
